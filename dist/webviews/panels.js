...
exports.ManageDataPanel = exports.ShowTipResultsPanel = exports.AddResultPanel = exports.AddTipPanel = exports.AddPlayerPanel = exports.AddRacePanel = void 0;
...
function createPanel(title, viewType) {
...
class AddRacePanel {
    static show(context) {
        ...
        panel.webview.html = (0, webviewUtils_1.webviewHtmlShell)({
    ...
    static show(context) {
      ...
      `,
        ...
        panel.webview.onDidReceiveMessage(async (msg) => {
            ...
            await (0, data_1.addPlayer)(context, { name });
            panel.webview.postMessage({ type: "status", kind: "ok", text: "Player saved." });
        });
    }
}
exports.AddPlayerPanel = AddPlayerPanel;
class AddTipPanel {
    static async show(context) {
        const data = await (0, data_1.loadData)(context);
        const panel = createPanel("Add tipp", "skiraceManager.addTip");
        const raceOptions = data.races
            .slice()
            .sort((a, b) => a.dateISO.localeCompare(b.dateISO))
            .map((r) => `<option value="${(0, webviewUtils_1.escapeHtml)(r.id)}">${(0, webviewUtils_1.escapeHtml)(`${r.dateISO} — ${r.gender} — ${r.name}`)}</option>`)
            .join("");
        const playerOptions = data.players
            .slice()
            .sort((a, b) => a.name.localeCompare(b.name))
            .map((p) => `<option value="${(0, webviewUtils_1.escapeHtml)(p.id)}">${(0, webviewUtils_1.escapeHtml)(p.name)}</option>`)
            .join("");
        panel.webview.html = (0, webviewUtils_1.webviewHtmlShell)({
            webview: panel.webview,
            title: "Add tipp",
      ...
      `,
        ...
        panel.webview.onDidReceiveMessage(async (msg) => {
            ...
            await (0, data_1.upsertTip)(context, { raceId, playerId, picks });
            panel.webview.postMessage({ type: "status", kind: "ok", text: "Tip saved." });
        });
    }
}
exports.AddTipPanel = AddTipPanel;
class AddResultPanel {
    static async show(context) {
        const data = await (0, data_1.loadData)(context);
      ...
      `,
        ...
        panel.webview.onDidReceiveMessage(async (msg) => {
            ...
            await (0, data_1.upsertResult)(context, { raceId, top5 });
            panel.webview.postMessage({ type: "status", kind: "ok", text: "Result saved." });
        });
    }
}
exports.AddResultPanel = AddResultPanel;
class ShowTipResultsPanel {
    static async show(context) {
        const data = await (0, data_1.loadData)(context);
        ...
        const sections = races.map((race) => {
            ...
            const rows = players.map((p) => {
                ...
                const points = tip && result ? (0, scoring_1.scoreTip)(tip.picks, result.top5) : 0;
                const status = !tip ? "No tip" : !result ? "Waiting result" : "Scored";
                return `<tr>
          <td>${(0, webviewUtils_1.escapeHtml)(p.name)}</td>
          <td>${points}</td>
          <td class="muted">${(0, webviewUtils_1.escapeHtml)(status)}</td>
        </tr>`;
            });
            const top5Text = result
                ? result.top5.map(webviewUtils_1.escapeHtml).join(", ")
                : "<span class='muted'>(no result yet)</span>";
            return `
        <h3>${(0, webviewUtils_1.escapeHtml)(`${race.dateISO} — ${race.gender} — ${race.name}`)}</h3>
...
exports.ShowTipResultsPanel = ShowTipResultsPanel;
class ManageDataPanel {
    static async show(context) {
        const data = await (0, data_1.loadData)(context);
        const panel = createPanel("Manage data", "skiraceManager.manageData");
        const initialDataJson = (0, webviewUtils_1.escapeHtml)(JSON.stringify(data));
        panel.webview.html = (0, webviewUtils_1.webviewHtmlShell)({
            webview: panel.webview,
            title: "Manage data",
            body: `
        ...
        panel.webview.onDidReceiveMessage(async (msg) => {
            ...
            try {
                if (type === "updatePlayer") {
                    ...
                    await (0, data_1.updatePlayer)(context, id, { name });
                    ...
                    await (0, data_1.deletePlayer)(context, id);
                    ...
                    const dateISO = toIsoDateOnly(String(msg.dateISO ?? ""));
                    if (!id || !name)
                        throw new Error("Race id and name are required.");
                    await (0, data_1.updateRace)(context, id, { name, gender, dateISO });
                    ...
                    await (0, data_1.deleteRace)(context, id);
                    ...
                    await (0, data_1.deleteTip)(context, id);
                    ...
                    await (0, data_1.deleteResult)(context, id);
                ...
                const updated = await (0, data_1.loadData)(context);
...
exports.ManageDataPanel = ManageDataPanel;
function sanitizeDataForWebview(data) {
